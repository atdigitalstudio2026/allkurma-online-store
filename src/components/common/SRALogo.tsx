...
export const SRALogo: React.FC<SRALogoProps> = ({
  ...
  // Dimension presets
  const sizeMap = {
    xs: { icon: 24, font: 'text-sm', sub: 'text-[9px]', gap: 'gap-1.5' },
    sm: { icon: 32, font: 'text-lg', sub: 'text-[10px]', gap: 'gap-2' },
  ...
  const KnotIcon = ({ iconSize }: { iconSize: number }) => (
    ...
    >
      {/* 
        SRA Interlocking Knot / Quad Loop Ribbon 
        Four symmetrical curved rounded loops forming an infinity-knot cross
      */}
      <g stroke={greenColor} strokeWidth="22" strokeLinecap="round" strokeLinejoin="round">
        {/* Top Loop */}
        <path
          d="M 68,96 L 68,60 C 68,40 82,24 100,24 C 118,24 132,40 132,60 L 132,96"
        />
        {/* Bottom Loop */}
        <path
          d="M 132,104 L 132,140 C 132,160 118,176 100,176 C 82,176 68,160 68,140 L 68,104"
        />
        {/* Left Loop */}
        <path
          d="M 96,132 L 60,132 C 40,132 24,118 24,100 C 24,82 40,68 60,68 L 96,68"
        />
        {/* Right Loop */}
        <path
          d="M 104,68 L 140,68 C 160,68 176,82 176,100 C 176,118 160,132 140,132 L 104,132"
        />
        {/* Center diagonal woven loops */}
        <path
          d="M 68,68 L 132,132"
        />
        <path
  ...
  if (variant === 'icon-only') {
    return (
      <div className={`inline-flex items-center justify-center ${className}`}>
        <KnotIcon iconSize={currentSize.icon} />
      </div>
    );
  }
  ...
  if (variant === 'stacked') {
    return (
      <div className={`flex flex-col items-center text-center ${currentSize.gap} ${className}`}>
        <div className="p-2.5 rounded-2xl bg-white shadow-xs border border-emerald-100 flex items-center justify-center">
          <KnotIcon iconSize={currentSize.icon} />
        </div>
        <div>
          <span 
